...
function AuthPage() {
  ...
  const validate = () => {
    ...
    if (tab !== "reset" && password.length < 6)
      e["password"] = ar ? "٦ أحرف على الأقل" : "At least 6 characters";
    if (tab === "signup" && name.trim().length < 3)
      e["name"] = ar ? "اكتب اسمك الكامل" : "Enter your full name";
    setErrors(e);
    ...
    try {
      if (tab === "reset") {
        ...
        if (res.error) toast.error(res.error);
          ...
          toast.success(
            ar ? "أرسلنا رابط إعادة التعيين إلى بريدك." : "Reset link sent to your email.",
          );
          setTab("signin");
        }
        return;
